  ...
  return {
    ...
    select(index, event) {
      ...
      if (!this.options[index].selected) {
        ...
        let addInput = `<input hidden name="listOfSkill" id="${this.options[index].text}" value="${this.options[index].value}"/>`
    ...
    loadOptions() {
      ...
      for (let i = 0; i < options.length; i++) {
        ...
        if (options[i].getAttribute('selected') === '') {
          ...
          let addInput = `<input hidden name="listOfSkill" id="${this.options[i].text}" value="${this.options[i].value}"/>`
          document.querySelector("#skills").insertAdjacentHTML('beforeend', addInput);
        }
      }
